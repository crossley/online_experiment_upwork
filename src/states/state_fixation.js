...
state_fixation.create = function() {
    ...
    const cursorAppearTw = TweenLite.to(this.game.cursor, 0.3, {
        css: {
            opacity: 1
        },
        ease: Power2.easeOut
    ...
    if (!configParams["no_animation_at_all"] && configParams["do_animate_fixation_pointer"]) {
        TweenLite.to(this.dot, 0.4, {
            css: {
                top: (window.innerHeight / 2 - this.dot.offsetHeight / 2 - 65).toString() + "px"
            },
    ...
    const instructTw = TweenLite.to(this.lblInstruct, 0.4, {
        css: {
...
state_fixation.update = function() {
    ...
    if (Math.abs(mouseX - configParams["fixation_point_position"][0]) < radTolX &&
        ...
        let circX = (window.innerWidth / 2 - this.circle.offsetWidth / 2) * ((this.curGuideRadius / this.guideRadius)) +
            (this.game.mouseX - this.curGuideRadius) * (1 - this.curGuideRadius / this.guideRadius);
        let circY = (window.innerHeight / 2 - this.circle.offsetHeight / 2) * ((this.curGuideRadius / this.guideRadius)) +
            (this.game.mouseY - this.curGuideRadius) * (1 - this.curGuideRadius / this.guideRadius);

    } else {
        this.circResetTw = TweenLite.to(this, 0.3, {
            curGuideRadius: this.guideRadius,
            ease: Power2.easeOut
        });
    }
