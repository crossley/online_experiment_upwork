...
state_trial.reposition = function(){
  if(this.curPhase == "pick") {
    ...
    for(let i = 0; i<this.stimuli.length; i++){
      ...
      stim.style.left = (window.innerWidth/2 + position[0] * window.innerWidth/2 - parseInt(stim.style.width) / 2).toString() + "px";
      stim.style.top = (window.innerHeight/2 + position[1] * window.innerHeight/2 - parseInt(stim.style.height) / 2).toString() + "px";
    }
...
state_trial.showCategorize = function(){
  this.lblInstruct = this.game.addLabel(window.innerWidth/2, window.innerHeight + 50, "Choose the appropriate category", 24);
  ...
  this.btnAction1 = this.game.addImage(window.innerWidth / 4, 3 * window.innerHeight / 4 + 100, "keyboardBtn");
  ...
  this.btnAction1.lbl.style.left = "45px";
  this.btnAction1.lbl.style.top =  "-15px";

  this.btnAction2 = this.game.addImage(3 * window.innerWidth / 4, 3 * window.innerHeight / 4 + 100, "keyboardBtn");
  ...
  this.btnAction2.lbl.style.left = "45px";
  this.btnAction2.lbl.style.top =  "-15px";
...
state_trial.elemenateOtherStimuli = function(){
  console.log("elemenateOtherStimuli");
  for(let i = 0; i<this.stimuli.length; i++){
    const stim = this.stimuli[i];
